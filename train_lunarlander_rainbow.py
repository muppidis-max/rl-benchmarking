...
def plot_results(rewards, seed):
    ...
    plt.title(f'Rainbow on LunarLander-v3 (Seed {seed})')
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'experiments/results/rainbow_lunarlander_seed{seed}.png', dpi=150)
    np.save(f'experiments/results/rainbow_rewards_lunar_seed{seed}.npy', np.array(rewards))
    plt.show()
    print(f"Plot saved.")
