...
def plot_results(rewards, seed):
    # rolling average over 50 episodes
    ...
    plt.title(f'DDQN on CartPole-v1 (Seed {seed})')
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'experiments/results/ddqn_cartpole_seed{seed}.png', dpi=150)
    np.save(f'experiments/results/ddqn_rewards_seed{seed}.npy', np.array(rewards))
    plt.show()
