...
def plot_results(rewards, seed):
    ...
    plt.title(f'DQN on CartPole-v1 (Seed {seed})')
    plt.legend()
    plt.tight_layout()
    plt.savefig('experiments/results/dqn_cartpole.png', dpi=150)
    # Add inside plot_results(), after rolling avg calculation:
    np.save(f'experiments/results/dqn_rewards_seed{seed}.npy', np.array(rewards))
    plt.show()
