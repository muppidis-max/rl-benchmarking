...
from agents.double_dqn import DDQNAgent
...
def plot_results(rewards, seed):
    ...
    plt.axhline(y=195, color='red', linestyle='--', label='Solved (200)')
    plt.xlabel('Episode')
    plt.ylabel('Total Reward')
    plt.title(f'DDQN on LunarLander-v3 (Seed {seed})')
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'experiments/results/ddqn_lunarlander_seed{seed}.png', dpi=150)
    np.save(f'experiments/results/ddqn_rewards_lunar_seed{seed}.npy', np.array(rewards))
    plt.show()
